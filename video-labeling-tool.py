...
class MyApp(QWidget):
    ...
    def changeImg(self):
        if self.isVideoLoaded or self.isDirLoaded:
            ...
            self.brightnessValue.setText(str(self.brightnessSlider.value()))
            self.contrastValue.setText(str(self.contrastSlider.value()))
            blk = np.zeros([self.h, self.w, self.ch], image.dtype)
            self.showImg = cv2.addWeighted(image, (self.contrastSlider.value() / 10), blk,
            ...
            for roi in self.rois:
                ...
                blk = np.zeros(self.copy.shape, np.uint8)
                cv2.rectangle(blk, (roi[0], roi[1]), (roi[2], roi[3]), (255, 255, 255), cv2.FILLED)
                self.copy = cv2.addWeighted(self.copy, 1.0, blk, 0.25, 1)
    ...
    def keyPressEvent(self, e):
        ...
        if e.key() == Qt.Key_Delete and self.imgLabel.sRoi:
            ...
            for roi in self.rois:
                ...
                blk = np.zeros(img_draw.shape, np.uint8)
                cv2.rectangle(blk, (roi[0], roi[1]), (roi[2], roi[3]), (255, 255, 255), cv2.FILLED)
                img_draw = cv2.addWeighted(img_draw, 1.0, blk, 0.25, 1)
            self.copy = img_draw
            self.loadImage(self.copy)
            self.imgLabel.setCursor(QCursor(QtCore.Qt.CrossCursor))
            ...
            if e.button() == Qt.LeftButton and self.ma.isClassLoaded:
                ...
                if self.sRoi and (self.sRoi[0] - 3 <= self.xStart <= self.sRoi[2] + 3) \
                    ...
                    for roi in self.ma.rois:
                        ...
                        blk = np.zeros(self.img_draw.shape, np.uint8)
                        cv2.rectangle(blk, (roi[0], roi[1]), (roi[2], roi[3]), (255, 255, 255), cv2.FILLED)
                        self.img_draw = cv2.addWeighted(self.img_draw, 1.0, blk, 0.25, 1)
                else:
                    self.draw = True
                    self.sRoi = []
    ...
    def mouseMoveEvent(self, e):
        ...
        if Qt.LeftButton:
            if self.moveRoi:
                ...
                blk = np.zeros(copy.shape, np.uint8)
                cv2.rectangle(blk, (self.sRoi[0], self.sRoi[1]), (self.sRoi[2], self.sRoi[3]), (255, 255, 255), cv2.FILLED)
                copy = cv2.addWeighted(copy, 1.0, blk, 0.25, 1)
                ...
                blk = np.zeros(img_draw.shape, np.uint8)
                cv2.rectangle(blk, (self.xStart, self.yStart), (x, y), (255, 255, 255), cv2.FILLED)
                img_draw = cv2.addWeighted(img_draw, 1.0, blk, 0.25, 1)
                ...
                if self.moveRoi:
                    ...
                    for roi in self.ma.rois:
                        if roi == self.sRoi:
                            ...
                            blk = np.zeros(self.ma.copy.shape, np.uint8)
                            cv2.rectangle(blk, (roi[0], roi[1]), (roi[2], roi[3]), (255, 255, 255), cv2.FILLED)
                            self.ma.copy = cv2.addWeighted(self.ma.copy, 1.0, blk, 0.25, 1)
                        else:
        ...
        if self.sRoi:
            ...
            for roi in self.ma.rois:
                if roi == self.sRoi:
                    ...
                    blk = np.zeros(img_draw.shape, np.uint8)
                    cv2.rectangle(blk, (self.sRoi[0], self.sRoi[1]), (self.sRoi[2], self.sRoi[3]), (255, 255, 255),
                                  cv2.FILLED)
                    img_draw = cv2.addWeighted(img_draw, 1.0, blk, 0.25, 1)
                else:
